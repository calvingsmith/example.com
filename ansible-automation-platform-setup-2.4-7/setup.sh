    ...
    case $OPTION in
        ...
        p)
            if [ -d $OPTARG ]; then
                ...
                log_warning "Provided path does not exist. Using the default log path."
            fi
            ;;
        e)
...
# Run the playbook.
touch $(dirname $0)/${TEMP_LOG_FILE}
chmod 0640 $(dirname $0)/${TEMP_LOG_FILE}
...
ansible-playbook -i "${INVENTORY_FILE}" -v \
                 $OPTIONS \
                 $PLAYBOOK 2>&1 | tee $(dirname $0)/${TEMP_LOG_FILE}
...
if [ -d "${LOG_DIR}" ]; then
    if [ -w "${LOG_DIR}" ]; then
       cp $(dirname $0)/${TEMP_LOG_FILE} ${LOG_FILE}
    else
       ${ANSIBLE_BECOME_METHOD:-sudo} cp $(dirname $0)/${TEMP_LOG_FILE} ${LOG_FILE}
    fi
    if [ $? -eq 0 ]; then
        rm $(dirname $0)/${TEMP_LOG_FILE}
        ...
        log_warning "Unable to save log to ${LOG_DIR}. Setup log saved to $(dirname $0)/${TEMP_LOG_FILE}."
    fi
else
...
if [ -f "${TEMP_LOG_FILE}" ]; then
    if [ -O "${TEMP_LOG_FILE}" ]; then
        chmod 0640 ${TEMP_LOG_FILE}
    else
        ${ANSIBLE_BECOME_METHOD:-sudo} chmod 0640 ${TEMP_LOG_FILE}
        ${ANSIBLE_BECOME_METHOD:-sudo} chown ${SUDO_USER} ${TEMP_LOG_FILE}
    fi
    if [ -n ${TEMP_LOG_DIR} ]; then
        if [ -d "${TEMP_LOG_DIR}" ] && [ -w "${TEMP_LOG_DIR}" ]; then
            mv $(dirname $0)/${TEMP_LOG_FILE} ${TEMP_LOG_DIR}/
        else
            log_warning "Provided path does not exist or is not accessible. Setup log saved to $(dirname $0)/${TEMP_LOG_FILE}."
        fi
    fi
fi
